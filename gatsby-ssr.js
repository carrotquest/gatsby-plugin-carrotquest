...
exports.onRenderBody = function (_ref, _ref2) {
  ...
  var delay = "if (document.documentElement.clientWidth < 700) { setTimeout(init, " + mobileDelay + "); } else { setTimeout(init, " + desktopDelay + "); }";
  var scriptInnerHTML = "window.carrotquestId = \"" + carrotquestId + "\";" + "if (typeof carrotquest === \"undefined\") {  function Build(name, args) {    return function () {      window.carrotquestasync.push(name, arguments);    };  }  window.carrotquest = {};  window.carrotquestasync = [];  carrotquest.settings = {};  var m = [    \"connect\",    \"track\",    \"identify\",    \"auth\",    \"open\",    \"onReady\",    \"addCallback\",    \"removeCallback\",    \"trackMessageInteraction\",  ];  for (var i = 0; i < m.length; i++) carrotquest[m[i]] = Build(m[i]);}" + "function init() {  var s = document.createElement(\"script\");  s.type = \"text/javascript\";  s.async = true;  s.src = \"//cdn.carrotquest.io/api.min.js\";  var x = document.getElementsByTagName(\"head\")[0];  x.appendChild(s);}" + delay;
