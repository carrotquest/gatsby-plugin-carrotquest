...
exports.onRenderBody = ({ setHeadComponents }, { carrotquestId, mobileDelay = 0, desktopDelay = 0 }) => {
  ...
  const delay = `if (document.documentElement.clientWidth < 700) { setTimeout(initCQ, ${mobileDelay}); } else { setTimeout(initCQ, ${desktopDelay}); }`;
  ...
  const scriptInnerHTML =
    ...
    `function initCQ() {  var s = document.createElement("script");  s.type = "text/javascript";  s.async = true;  s.src = "//cdn.carrotquest.io/api.min.isolate.js";  var x = document.getElementsByTagName("head")[0];  x.appendChild(s);}` +
    delay;
